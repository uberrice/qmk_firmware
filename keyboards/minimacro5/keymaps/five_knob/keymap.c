...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = { //buttion closest to usb is first
  [_MAIN] = LAYOUT_ortho_1x5(
     KC_RGHT, KC_LEFT, KC_UP, KC_DOWN, KC_E
  )
};
 ...
 */
    ...
    const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {              
        [_MAIN] =  { 
          ...
          /* Selects adjacent words. (Command + Shift + Right Arrow). */
          ENCODER_CCW_CW(C(S(KC_LEFT)), C(S(KC_RGHT))),
          /* Jumps to end/start of line. Hold shift to select. (Gui + arrow). */
